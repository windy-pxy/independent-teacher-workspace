...
最后核实：2026-09-12（Asia/Shanghai）。新会话先读根 `AGENTS.md` 和本文件，再核对 Git、容器、迁移和工作区；本文件是快照，不替代真实状态。

## 1. 项目当前状态

- 目录：`C:\Users\wyf\Documents\教培个人工作流`。
- 产品正从单教师本机工作台升级为“多个独立教师各自注册、登录并管理私人教学资料”的在线工作台。
- 原 Phase 0—8 已验收。多教师升级使用 M1—M5：M1、M2、M3 已提交推送；M4 代码、自动验收和保留数据卷的本地升级已完成，正等待差异检查、提交与推送；M5 云部署未开始。
- 当前分支 `main`，私有远程 `https://github.com/windy-pxy/independent-teacher-workspace.git`。
- 当前 HEAD / `origin/main`：`9cbef843a831d6da52fc0e1acbe78b28f2531a0a`，提交说明为 M3 全业务隔离与持久化验收。
- 当前 Compose 已运行 M4：Web/API/数据库健康，Worker 运行，数据库迁移头为 `20260911_0009`。升级前后账户、学生、学科、课程和资料数量一致，过程中未删除卷。
...
1. 检查 M4 `git diff`、未跟踪文件和敏感信息，明确排除旧 `README.md`/`docs/go-live-checklist.md`。
2. 创建 M4 本地提交并推送现有私有 `origin/main`（用户已有持续授权，禁止强推）。

### M5 云部署与首次多人试用

1. 用户按 `docs/cloud-purchase-guide.md` 购买合适服务器和域名；购买、备案和提供真实支持联系方式必须由用户完成。
2. 配置 Ubuntu、SSH 最小权限、防火墙、Docker、HTTPS、生产密钥和异地加密备份。
3. 把本机正式 PostgreSQL/Storage 迁移到唯一云端主库，迁移前后核对并避免双写。
4. 验证云服务器重启自启、备份恢复、另一电脑/手机通过真实域名登录和持久化。
5. 先邀请少量独立教师，以新用户视角记录困惑并修复，再决定是否扩大开放。
...
若 M4 尚未提交，按“待办事项”收尾。若 M4 已推送且本地迁移为 `20260911_0009`，进入 M5，并先让用户购买/提供云服务器、域名和真实支持联系方式；不要擅自购买外部资源。
