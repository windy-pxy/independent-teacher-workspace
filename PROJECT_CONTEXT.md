...
- 2026-09-12 一次生产 Compose 诊断错误地展开了本地环境变量。已立即创建并校验备份、轮换数据库密码和会话密钥、清空本地 DeepSeek/千问 Key，并把文本/视觉 Provider 切回 Mock；服务和迁移恢复健康。用户仍须在 DeepSeek 与阿里云百炼后台撤销旧 Key，之后只在本机 `.env` 填入新 Key。
...
2. 用户在 DeepSeek 与阿里云百炼控制台撤销已暴露的旧 API Key；需要真实 AI 时创建新 Key 并仅写入本机/服务器 `.env`，不得通过聊天发送。
