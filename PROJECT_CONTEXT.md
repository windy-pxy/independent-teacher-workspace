...
- M1、M2 已提交并推送；M2 已安全部署到现有本地运行环境。M3 已完成开发和全部自动验收，等待提交与推送；M4—M5 未完成。
- 用户已于 2026-09-11 明确要求读取交接文件并继续，已恢复 M2 收尾；不购买或部署未授权的外部资源。
- 目标模式的总体目标仍未完成，不可把本次交接或 M2 局部验证当作多教师平台最终完成。

### 2026-09-11 续跑最新记录（优先于下方历史）

- M2 提交 `01c5ccc` 已推送私有 `origin/main`。使用保留原 PostgreSQL/Storage 卷的 Compose 重建完成，迁移头为 `20260910_0008`，Web/API 健康；部署前备份仍在忽略目录。
- M3 新增标签页账户上下文核对和 BroadcastChannel 换号保护，旧标签页不能把资料写入新 Cookie 对应的账户。
- M3 新增完整双教师隔离测试，覆盖各业务域、跨 ID/关联/下载/导出、并发版本冲突和越权后原记录不变。
- M3 真实 Edge 流程已通过：两人注册和隔离保存、API/Web 重启后持久化、未保存离开提醒、并发冲突提示、同浏览器多标签换号阻断，页面异常 0。
- 新手测试发现并修复学生编辑/归档判断错位：有排课时允许编辑基础资料，但仍阻止误归档。
- M3 `pnpm check:release` 已退出 0：Web 16 项、后端 60 项通过/1 项 PG 专用普通运行跳过、lint、全部类型检查、OpenAPI、前后端生产构建、静态扫描、Node/Python 依赖漏洞扫描和三套 Compose 配置均通过。PG 专用隔离测试已在独立 PostgreSQL 中通过。
...
- 下一步：核对 M3 差异与敏感数据，按持续授权提交推送；随后进入 M4。不要把 M3 当成公网开放完成。

### Git

- 分支：`main`。
- 现有私有远程：`https://github.com/windy-pxy/independent-teacher-workspace.git`。
- 最近提交：`01c5ccc` — `feat: 完成多教师 M2 注册与账户安全并修复依赖漏洞`。已推送，远程 SHA 已核实一致。
- 前一提交：`45b34e0` — M1 会话隔离；再前为 `f3f5235` — 工作台界面与内嵌编辑。
- 工作区现为 M3 更改，等待完整发布检查与提交。不要 reset、checkout 或覆盖恢复。
...
- 现有 `teacher-workspace-db-1`、`api-1`、`web-1` 健康；`worker-1` 运行。M2 镜像和迁移已部署，M3 尚未部署。
- Web 使用 `http://localhost:3000`；现有数据库和存储含用户实际资料，必须保留。
- 本次通过迁移容器把现有资料库升级到 M2 迁移头；部署前已备份，未删除或重建卷，没有调用付费 AI。
...
- M3 代码与真实 PostgreSQL/Edge 验收已通过；仍需完整发布检查、差异/敏感扫描、提交推送和本地容器部署。
- AI 权限管理/费用/配额、防滥用和后台任务权限；目前 `User.ai_access_enabled` ORM 默认仍为 true（注册显式 false、数据库默认 false），M4 需改为默认拒绝并明确可信初始化与测试设置。
