...
4. Worker 校验结构化模型输出并写入不可变草稿版本；人工审核接口只批准指定版本，不覆盖历史。
...
- `AIProvider` 接收指令、最小化上下文和 JSON Schema；领域服务不直接导入 OpenAI SDK。
- 开发/测试固定使用 Mock；OpenAI 适配器使用官方 SDK 与 Responses API，并关闭提供商侧响应存储。
...
- `LessonDocument` 是课程的逻辑教案，`DocumentVersion` 保存不可变结构化章节内容和审核状态。
- 完整生成和局部重生成都进入 PostgreSQL 队列；任务幂等键和 `ai_job_id` 唯一约束防止重复落版。
- Word 导出只接受已批准版本。DOCX 由后端从结构化内容确定性生成，题目与答案强制分区。
- 模板样式通过年级档案选择，当前不把特定学校模板写死；未来只替换渲染层，不迁移教案内容。
